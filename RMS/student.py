...
class studentClass:
  def __init__(self,root):
    ...
    #----TItle----
    title=Label(self.root,text="Mange Student Details",font=("goudy old style",20,"bold"),bg="#033053",fg="white").place(x=10,y=15,width=1180,height=35)
...
#-----column2-----
    ...
    #---------entry field------------
    self.course_list=[]
    #funcation_call to upfate the list
    self.fetch_course()
...
#-------------------------------------------------------------------------
  ...
  def delete(self):
    ...
    try:
        if self.var_roll.get() == "":
            ...
            row = cur.fetchone()
            if row is None:
                messagebox.showerror("Error", "Invalid Student from the list", parent=self.root)
            else:
  ...
  def add(self):
    ...
    try:
        if self.var_roll.get() == "":
            ...
            row=cur.fetchone()
            if row!=None:
               messagebox.showerror("Error", "Roll No.  alredy present", parent=self.root)
            else:
               ...
               con.commit()
               messagebox.showinfo("Seccess","Student Added Seccussfully",parent=self.root)
               self.show()
    except Exception as ex:
    ...
    try:
        if self.var_roll.get() == "":
            ...
            if row == None:
               ...
               con.commit()
               messagebox.showinfo("Seccess","Student Update  Seccussfully",parent=self.root)
               self.show()
    except Exception as ex:
        messagebox.showerror("Error", f"Error due to {str(ex)}")
  ...
  def search(self):
    ...
    try:
        ...
        # Correct SQL: Search by roll number
        ...
        if row is not None:
            ...
            messagebox.showinfo("No Result", "No record found with this Roll Number", parent=self.root)
    except Exception as ex:
        messagebox.showerror("Error", f"Error due to {str(ex)}", parent=self.root)
