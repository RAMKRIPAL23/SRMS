...
class resultClass:
  def __init__(self,root):
    ...
    #-----buttton---  
    btn_add=Button(self.root,text="Submit", font=("times new roman",15), bg="lightgreen",activebackground="lightgreen",cursor="hand2",command=self.add).place(x=300, y=420,width=120,heigh=35)
    ...
    try:
        if self.var_course.get() == "":
            ...
            if row!=None:
              ...
              con.commit()
              messagebox.showinfo("Seccess","Result Added Seccussfully",parent=self.root)
    except Exception as ex:
        messagebox.showerror("Error", f"Error due to {str(ex)}")
