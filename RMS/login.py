      ...
      if self.cmb_quest.get()=="select" or self.txt_answer.get()=="" or self.txt_new_pass.get()=="":
        messagebox.showerror("Error","All Fields are required",parent=self.root2)
          ...
          try:
            ...
            print(row)
            if row==None:
                messagebox.showerror("Error","Please Enter Select the Correct Security Question / Answer",parent=self.root2)
            else:
              cur.execute("update employee set password=? where email=?",(self.txt_new_pass.get(),self.txt_email.get()))
              con.commit()
              con.close()
              messagebox.showinfo("Seccuss","Your Password has been reset / Please login with new passsword",parent=self.root2)
              self.reset()
              self.root2.destroy()
    ...
    if self.txt_email.get()=="" or self.txt_pass_.get()=="":
      ...
      try:
          ...
          if row==None:
             ...
             messagebox.showinfo("Seccuss",f"Welcome:    {self.txt_email.get()}",parent=self.root)
             self.root.destroy()
